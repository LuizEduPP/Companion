...
async function readKwinFocusFromJournal() {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i];
    const idx = line.indexOf("COMPANION_FOCUS_JSON:");
    if (idx < 0) continue;
    try {
      return parseFocusPayload(JSON.parse(line.slice(idx + "COMPANION_FOCUS_JSON:".length)));
    } catch {
      /* keep scanning */
    }
  }
...
async function focusViaKWinScript() {
  ...
  await sh(
    qd,
    ["org.kde.KWin", "/Scripting", "org.kde.kwin.Scripting.unloadScript", "companion-focus"],
    { timeout: 1500 },
  );
  ...
  // Optional run path (often missing on Wayland; script may still execute on load).
  const tried = new Set();
  ...
  if (!parsed?.caption && !parsed?.resourceClass && !parsed?.pid) {
    // writeConfig missing + print not scraped → stop retrying every poll.
    kwinScriptDisabled = true;
...
async function pidFromAppName(appName) {
  ...
  try {
    ...
    for (const ent of procs) {
      ...
      try {
        const comm = (await readFile(`/proc/${ent}/comm`, "utf8")).trim().toLowerCase();
    ...
    // Prefer youngest (highest pid) when ambiguous.
...
try:
  ...
  skip = {'xembedsniproxy','gmenudbusmenuproxy','kaccess','foreground-booster','kded6','ksecretd','kwalletd','kimpanel-ibus-panel','baloorunner','ActivityManager','xdg-desktop-portal-gtk','xdg-desktop-portal-kde','kdeconnect.daemon','polkit-kde-authentication-agent-1'}
  out = []
  for i in range(desk.get_child_count()):
    ...
    for j in range(min(n, 40)):
      ...
      if not nm: continue
      if not showing and 'plasma' in an.lower(): continue
      out.append({'app': an[:80], 'title': nm[:160], 'pid': None})
      if len(out) >= 12: break
    if len(out) >= 12: break
  ...
  print('[]')
`;
  const r = await sh("python3", ["-c", script], { timeout: 3000 });
...
export async function getFocus() {
  // AT-SPI first on this host: KWin dbus scripting lacks writeConfig (Plasma 6).
  try {
    const a11y = await getA11yFocus();
    if (a11y && (a11y.app || a11y.title)) {
      let pid = null;
      const viaKwin = kwinScriptDisabled ? lastKwinFocus : await focusViaKWinScript();
  ...
  const viaScript = kwinScriptDisabled ? lastKwinFocus : await focusViaKWinScript();
  if (viaScript && viaScript.app !== UNKNOWN_APP) return viaScript;
  if (viaScript) return viaScript;
  return null;
}

export async function listWindows() {
  // Always refresh — stale cache hid new window titles after the first poll.
...
export async function getClipboardText() {
  // Clipboard portal / wl-clipboard ship with many Wayland desktops.
  for (const [cmd, args] of [
    ["wl-paste", ["--no-newline"]],
    ["xclip", ["-selection", "clipboard", "-o"]],
    ["xsel", ["--clipboard", "--output"]],
  ]) {
    const r = await sh(cmd, args, { timeout: 1500 });
    if (r.ok && r.stdout) return r.stdout.slice(0, 16000);
  }
  return "";
}

/** Primary selection (mouse-selected text on X11/Wayland). */
...
async function ensureA11yEnabled() {
  // Plasma/Wayland often leaves a11y off until a screen reader asks — without
  // this, AT-SPI only sees proxy stubs and typed/selection never appear.
  const script = `
import sys
try:
  import dbus
  ...
  print('skip', e)
`;
  await sh("python3", ["-c", script], { timeout: 2000 });
}

let a11yReady = false;

export async function getA11yFocus() {
  // AT-SPI via python3+gi — focus app/title + text value + in-widget selection.
  if (!a11yReady) {
    await ensureA11yEnabled();
    a11yReady = true;
  }
...
try:
  ...
  def read_text(acc, limit=8000):
    if acc is None: return ''
    try:
      if hasattr(acc, 'is_text') and not acc.is_text():
        # still try — some roles expose Text without is_text
        pass
    except Exception:
      pass
    try:
  ...
  def read_selection(acc, limit=8000):
    if acc is None: return ''
    try:
    ...
    try:
      ...
      if sels:
        # list of TextRange or (start,end)
        first = sels[0]
  ...
  for i in range(desk.get_child_count()):
    ...
    try:
      ...
      an = ''
    # Skip a11y bus stubs
    if an in ('xembedsniproxy', 'gmenudbusmenuproxy', 'kaccess'):
      continue
  ...
  # Focused node is often a tab/chrome; prefer a texty descendant or frame.
  text_acc = focused
  ...
  try:
    ...
    pass
  if not password:
    value = read_text(text_acc, 8000)
    selection = read_selection(text_acc, 8000)
  title = ''
  frame_title = ''
  cur = focused
  for _ in range(24):
  ...
  print('')
`;
  const r = await sh("python3", ["-c", script], { timeout: 3500 });
  if (!r.ok || !r.stdout) return null;
  try {
  ...
  try {
    ...
    for (const fd of fds.slice(0, 96)) {
      try {
        ...
        // Prefer home files, but keep other absolute docs (e.g. /mnt, /media).
        if (
