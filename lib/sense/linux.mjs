...
async function pasteVia(cmds) {
  for (const [cmd, args] of cmds) {
    ...
    if (r.ok && r.stdout) return r.stdout.slice(0, IO.clipboardRaw);
  }
  return "";
}
...
try:
  ...
  for i in range(desk.get_child_count()):
    ...
    for j in range(min(n, 40)):
      ...
      out.append({'app': an[:${IO.windowApp}], 'title': nm[:${IO.windowTitle}], 'pid': None})
...
export async function listOpenFiles(pid) {
  ...
  try {
    ...
    for (const fd of fds.slice(0, 96)) {
      try {
        ...
        out.push(target.slice(0, IO.path));
      } catch {
        /* ignore */
      }
    }
  ...
  return [...new Set(out)].slice(0, IO.openFiles);
}
