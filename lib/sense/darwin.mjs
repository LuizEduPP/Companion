import { sh, packFocus, listRecentFiles } from "./util.mjs";
...
export async function getFocus() {
  ...
  const r = await sh("osascript", ["-e", script], { timeout: 5000 });
  if (!r.ok) return null;
...
export async function getClipboardText() {
  const r = await sh("pbpaste", [], { timeout: 1500 });
  return r.ok ? r.stdout.slice(0, 16000) : "";
}
...
`;
  const r = await sh("osascript", ["-e", script], { timeout: 4000 });
  ...
  return {
    name: parts[0] || "",
    role: parts[1] || "",
    value: password ? "" : (parts[2] || "").slice(0, 8000),
    selection: "",
...
export async function listWindows() {
  ...
  return r.stdout
    ...
    .map((line) => {
      ...
      return { app: app.slice(0, 80), title: title.slice(0, 160), pid: null };
    });
}
