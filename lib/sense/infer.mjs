...
export function inferFromTitle(app, title) {
  ...
  if (urlMatch) {
    out.url = urlMatch[0].replace(/[.,;]+$/, "").slice(0, IO.url);
    out.signals.push("url_in_title");
  }

  const parts = t.split(TITLE_SPLIT).map((s) => s.trim()).filter(Boolean);
  if (parts.length) {
    out.title_segments = parts.map((s) => s.slice(0, IO.fileHint));
    out.signals.push("title_segments");
    if (parts.length >= 2) {
      out.file_hint = parts[0].slice(0, IO.fileHint);
    }
  }

  const pathMatch = t.match(PATH_IN_TITLE);
  if (pathMatch) {
    out.path_hint = pathMatch[1].slice(0, IO.path);
