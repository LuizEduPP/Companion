...
export function inferKind(_app, title, url, file) {
  ...
  if (PATH_IN_TITLE.test(String(title || ""))) return "editor";
  return "other";
}
  ...
  if (pathMatch) {
    ...
    if (segs.length >= 2) {
      ...
      if (!looksLikeAppChrome(folder, a)) {
        out.project_guess = folder.slice(0, LIMITS.project);
      }
    }
