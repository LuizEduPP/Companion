 ...
 * Do NOT blank whole project paths just because the folder is named companion.
 */
export function isInfraNoise(text) {
  ...
  // Env dumps / API keys — not human writing.
  if (
  ...
  // Own memory.json / episode JSON blobs.
  ...
  if (lines.length >= 3) {
    const noisy = lines.filter((l) =>
      /\[(?:hot|brain|sense)\]|think JSON|companion (?:brain|sense|hot)/i.test(l),
    ).length;
