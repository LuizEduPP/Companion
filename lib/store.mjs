...
import { isInfraNoise } from "./sense/util.mjs";
...
function hygieneEpisodes(episodes) {
  ...
  for (const raw of episodes || []) {
    ...
    if (e.type === "pc_typed" || e.type === "pc_a11y" || e.type === "pc_clipboard") {
      ...
      if (typeof e.text === "string") e.text = e.text.slice(0, 280);
      if (typeof e.value === "string") e.value = e.value.slice(0, 280);
      if (typeof e.selection === "string") e.selection = e.selection.slice(0, 200);
    }
...
export function appendEpisode(event) {
  ...
  if (
    ...
    if (typeof row.text === "string") row.text = row.text.slice(0, 280);
    if (typeof row.value === "string") row.value = row.value.slice(0, 280);
  }
...
export function patchUserProfile(patch = {}) {
  ...
  if (typeof patch.notes === "string" && patch.notes.trim()) {
    ...
    if (!String(cur.notes || "").includes(note)) {
      next.notes = [cur.notes, note].filter(Boolean).join(" · ").slice(0, 800);
      changed.add("notes");
    }
  }
...
export function applyLearn(learn) {
  ...
  if (!Array.isArray(learn.knows)) return;
  for (const f of learn.knows) {
    const text = typeof f === "string" ? f : f?.text;
    if (text) upsertKnow({ text: String(text) });
  }
}
...
function isJunkFact(text) {
  ...
  // Companion talking about itself / its own stack.
  if (/\b(companion)\b/.test(t) && /(terminal|process|brain|sense|orb|component)/i.test(t)) {
    return true;
  }
  return false;
}
