...
/** Compiled memory: user + knows[] + episodes + intent. */
const EMPTY_STORE = {
  user: null,
  knows: [],
  episodes: [],
...
function normalizeIntent(raw) {
  ...
  return {
    goals: asLines(raw.goals),
    commitments: asLines(raw.commitments),
    last_outcome:
      raw.last_outcome && typeof raw.last_outcome === "object"
        ? raw.last_outcome
        : null,
  };
}
...
function normalizeStore(raw) {
  ...
  for (const key of SECTION_KEYS) {
    ...
    const val = raw?.[key];
    if (key === "intent") {
      out.intent = normalizeIntent(val);
      continue;
    }
    if (Array.isArray(def)) {
      ...
      out[key] = val ?? structuredClone(def);
    }
  }
  return hygieneStore(out);
}

/** Exact-case-insensitive know dedupe; keep slim user. */
function hygieneStore(store) {
  ...
  for (const text of lines) {
    ...
    const twin = kept.findIndex(
      (k) => String(k.text || "").trim().toLowerCase() === norm,
    );
    if (twin >= 0) {
  ...
  store.intent = normalizeIntent(store.intent);
  return store;
}
...
function hygieneEpisodes(episodes) {
  ...
  for (const raw of episodes || []) {
    ...
    if (!EPISODE_TYPES.has(raw.type)) continue;
    const e = { ...raw };
...
export function appendEpisode(event) {
  ...
  if (!EPISODE_TYPES.has(event.type)) return null;
  const row = { ...event, at: event.at ?? new Date().toISOString() };
  ...
  updateStore((store) => {
    ...
    store.episodes = [...store.episodes, row];
    return store;
  });
  return row;
}
...
export function loadStore() {
  ...
  try {
    ...
    // Persist hygiene (exact know dedupe) so disk matches runtime.
...
export function patchUserProfile(patch = {}) {
  ...
  if (typeof patch.notes === "string" && patch.notes.trim()) {
    ...
    if (!String(cur.notes || "").includes(note)) {
      next.notes = [cur.notes, note].filter(Boolean).join(" · ");
      changed.add("notes");
    }
  }
  ...
  if (Array.isArray(patch.gaps)) {
    ...
    if (gapsChanged) {
      next.gaps = [...byId.values()].filter((g) => !g.filled);
      changed.add("gaps");
    }
  }
...
export function getIntent() {
  return normalizeIntent(loadStore().intent);
}

/** Merge model intent patch; omit fields to leave unchanged; [] clears list. */
export function applyIntent(patch) {
  if (patch == null) return getIntent();
  if (typeof patch !== "object" || Array.isArray(patch)) return getIntent();
  let next;
  updateStore((store) => {
    const cur = normalizeIntent(store.intent);
    next = { ...cur };
    if ("goals" in patch) next.goals = normalizeIntent({ goals: patch.goals }).goals;
    if ("commitments" in patch) {
      next.commitments = normalizeIntent({
        commitments: patch.commitments,
      }).commitments;
    }
    if ("last_outcome" in patch) {
      next.last_outcome =
        patch.last_outcome && typeof patch.last_outcome === "object"
          ? patch.last_outcome
          : null;
    }
    store.intent = next;
...
export function setLastOutcome(outcome) {
  return applyIntent({ last_outcome: outcome ?? null });
}
...
export function getWorldSnapshot() {
  ...
  return {
    ...
    knows: (store.knows || []).map((k) =>
      typeof k === "string" ? k : String(k?.text || ""),
    ),
    intent: normalizeIntent(store.intent),
  };
}
