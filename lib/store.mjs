...
const EPISODE_CAP = config.episodeCap;
...
function resolvedLocale() {
  const fromEnv = String(config.locale || "").trim();
...
function hygieneStore(store) {
  ...
  store.knows = kept.slice(-LIMITS.knowsCap);
...
export function recentEpisodes(limit = config.recentEpisodesLimit) {
...
function normalizeUser(prev, host) {
  if (!prev || typeof prev !== "object") {
    return {
      ...EMPTY_USER,
      name: String(config.userName || "").trim(),
...
export function upsertKnow({ id, text } = {}) {
  ...
  updateStore((store) => {
    ...
    if (idx >= 0) {
      ...
      store.knows = knows.slice(-LIMITS.knowsCap);
    }
    return store;
  });
...
function factTokens(text) {
  const minLen = config.knowTokenMinLen;
  return String(text || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/\p{M}/gu, "")
...
export function factsSimilar(a, b) {
  ...
  if (
    inter >= config.knowSimilarUnionMin &&
    inter / union >= config.knowSimilarUnionRatio
  ) {
    return true;
  }
  if (
    inter >= config.knowSimilarOverlapMin &&
    inter / Math.min(ta.size, tb.size) >= config.knowSimilarOverlapRatio
  ) {
    return true;
  }
  return false;
...
/** True when the note is mostly restating known profile fields. */
function isEchoFact(text, user) {
  ...
  for (const piece of [
    ...
    const p = String(piece || "")
      .trim()
      .toLowerCase();
    if (p) residual = residual.split(p).join(" ");
  }
  const compact = residual.replace(/[^a-z0-9]+/g, "");
  return compact.length < config.knowMinLen;
}

/** Structural only — phrase catalogs live in the prompt, not here. */
function isJunkFact(text) {
  const clean = String(text || "").trim();
  if (clean.length < config.knowMinLen) return true;
  if (isInfraNoise(clean)) return true;
  return false;
}
