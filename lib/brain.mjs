...
/** Context packing for the model payload — wire size, not speech policy. */
const PROMPT_EPISODES = 32;
const PROMPT_KNOWS = 24;
const PROMPT_GAPS = 12;
...
async function thinkAlive({
  ...
  const payload = {
    ...
    windows,
    user: userProf,
    gaps: (world.gaps ?? []).slice(0, PROMPT_GAPS),
    knows: (world.knows ?? []).slice(-PROMPT_KNOWS),
    episodes: (episodes ?? []).slice(-PROMPT_EPISODES),
    focus,
    inference,
    activity: {
  ...
  try {
    ...
    try {
      decision = await thinkAlive({
        world,
        episodes: recentEpisodes(PROMPT_EPISODES),
...
export function getUiState() {
  ...
  return {
    ...
    scene_digest: state.scene
      ...
      : null,
    knows: getWorldSnapshot().knows.slice(-PROMPT_KNOWS),
  };
}
