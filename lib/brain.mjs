...
async function thinkAlive({
  ...
  const payload = {
    ...
    windows,
    user: userProf,
    gaps: (world.gaps ?? []).slice(0, LIMITS.promptGaps),
    knows: (world.knows ?? []).slice(-LIMITS.promptKnows),
    episodes: (episodes ?? []).slice(-LIMITS.promptEpisodes),
    focus,
    inference,
    activity: {
    ...
    situation: {
      ...
      autonomous: true,
      sensor_thin: thin,
      has_human_content: Boolean(hc.sel || hc.clip || hc.typed),
  ...
  console.warn(
    ...
    String(content).slice(0, LIMITS.logSnippet),
  );
  return {
    silence: true,
    speak: null,
...
export function noteActivity(payload = {}) {
  ...
  if (payload.a11y && (payload.a11y.name || payload.a11y.value || payload.a11y.selection)) {
    const rawValue = payload.a11y.password_field
      ? ""
      : String(payload.a11y.value || "").slice(0, LIMITS.a11yValue);
    const rawSel = payload.a11y.password_field
      ? ""
      : String(payload.a11y.selection || "").slice(0, LIMITS.a11ySelection);
  ...
  if (Array.isArray(payload.recent_files) && payload.recent_files.length) {
    ...
    if (changed) {
      appendEpisode({
        type: "pc_recent",
        files: next.slice(0, LIMITS.recentFiles),
      });
      events.push("recent");
    }
  }
  ...
  if (Array.isArray(payload.open_files)) {
    ...
    if (changed && next.length) {
      appendEpisode({
        type: "pc_open_files",
        files: next.slice(0, LIMITS.openFiles),
      });
  ...
  return {
    ...
    activity: {
      ...
      typed: state.activity.typed
        ? {
            ...state.activity.typed,
            text: String(state.activity.typed.text).slice(
              0,
              LIMITS.statusTyped,
            ),
          }
      ...
      a11y: state.activity.a11y
        ...
        : null,
      recent_files: state.activity.recent_files?.slice(0, LIMITS.statusList),
      open_files: state.activity.open_files?.slice(0, LIMITS.statusList),
    },
    lastSpeak: state.lastSpeak,
