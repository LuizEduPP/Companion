...
async function chatCompletions({ model, messages, temperature }) {
  ...
  if (!content) {
    throw new Error(
      `empty model content (finish=${data?.choices?.[0]?.finish_reason || "?"})`,
    );
  }
  return content;
}
...
function signalFingerprint() {
  ...
  return [
    ...
    hc.clip || "",
    hc.sel || "",
    String(hc.typed || "").slice(0, LIMITS.fingerprintTyped),
    hc.page || "",
    hc.file || "",
    wins,
  ].join("\n");
...
/** Structural speak gate — policy lives in the prompt; no phrase catalogs here. */
function sanitizeSpeak(text) {
  const speak = String(text ?? "").trim().slice(0, LIMITS.speak);
  if (!speak) return null;
  if (mentionsModelId(speak)) return null;
  if (!config.speakAllowQuestions && /\?/.test(speak)) return null;
  return speak;
}
...
async function thinkAlive({
  ...
  const system = [
    ...
    "",
    "Follow the OUTPUT CONTRACT in the system prompt exactly. Single-line JSON only.",
  ].join("\n");
  const userProf = world.user
    ? {
        name: world.user.name || "",
        locale: world.user.locale || "",
        notes: String(world.user.notes || "").slice(0, LIMITS.notes),
      }
    : {};
  ...
  const user = [
    ...
    "Gaps:",
    JSON.stringify((world.gaps ?? []).slice(0, LIMITS.promptGaps)),
    "Knows:",
    JSON.stringify((world.knows ?? []).slice(-LIMITS.promptKnows)),
    "Episodes:",
    JSON.stringify((episodes ?? []).slice(-LIMITS.promptEpisodes)),
    "Focus:",
    ...
    JSON.stringify({
      ...
      open_files: (state.activity.open_files || []).slice(0, LIMITS.openFiles),
      windows,
    ...
    lastSpeak
      ? "If you would say something close to Last balloon, set silence=true (speak null)."
      : null,
    "Knows are durable beliefs — habits/preferences/tools/patterns. learn.knows every turn ([] ok). Never rephrase an existing Know. Never learn current focus/file/clipboard path, model ids, or companion meta.",
    thin
    ...
    "When speaking, emotion must be curious|focused|speak|happy|smug|wink|love|shy — not thinking.",
    "Reply with the single-line JSON object only. First char { last char }.",
  ]
    .filter(Boolean)
    .join("\n");
  ...
  console.warn(
    "[companion:think-llm] invalid JSON from model → silence:",
...
function sceneFromFocus(focus, inference) {
  ...
  return {
    ...
    typed_excerpt: hc.typed
      ? String(hc.typed).slice(0, LIMITS.modelTyped)
      : null,
    selection_excerpt: hc.sel
      ? String(hc.sel).slice(0, LIMITS.episodeSelectionLong)
      : null,
    clipboard_excerpt: hc.clip
      ? String(hc.clip).slice(0, LIMITS.modelContent)
      : null,
    a11y_excerpt: hc.a11y
      ? String(hc.a11y).slice(0, LIMITS.episodeSelectionLong)
      : null,
    open_files: (state.activity.open_files || []).slice(0, LIMITS.openFiles),
    windows: (state.activity.windows || []).slice(0, LIMITS.windowsMax),
...
async function drainTurn() {
  ...
  // Same sensors as last think → skip duplicate *sensor* turns only.
  // Timed turns (boot/proactive/nudge) must still call the model, or the
  // orb stays idle forever after the first fingerprint is locked.
  const fp = signalFingerprint();
  const timed =
    turn.reason === "nudge" ||
    turn.reason === "proactive" ||
    turn.reason === "boot";
  ...
  state.awaitingUser = false;
  // Do not flip to "thinking" every proactive tick — morph thrash looks like tremor.
  ...
  try {
    ...
    try {
      decision = await thinkAlive({
        scene: state.scene,
        world,
        episodes: recentEpisodes(config.recentEpisodesLimit),
    ...
    const spokeAgo = state.lastSpokeAt
      ? Date.now() - state.lastSpokeAt
      : Number.POSITIVE_INFINITY;
    if (
      !nudged &&
      decision.speak &&
      config.speakMinMs > 0 &&
      spokeAgo < config.speakMinMs
    ) {
      decision = {
        ...decision,
        silence: true,
    ...
    // Drop near-duplicate / paraphrase of last balloon.
    if (
      decision.speak &&
      state.lastSpeak &&
      (decision.speak.toLowerCase().replace(/\s+/g, " ") ===
        state.lastSpeak.toLowerCase().replace(/\s+/g, " ") ||
        factsSimilar(decision.speak, state.lastSpeak))
    ) {
      decision = {
        ...decision,
        silence: true,
...
function maybeProactive() {
  ...
  // Respect speak cooldown — do not burn the model just to mute again.
  if (
    config.speakMinMs > 0 &&
    state.lastSpokeAt &&
...
export function getUiState() {
  ...
  return {
    ...
    ui_poll_ms: config.uiPollMs,
    hot_reload_poll_ms: config.hotReloadPollMs,
    scene_digest: state.scene
      ...
      : null,
    knows: getWorldSnapshot().knows.slice(-LIMITS.promptKnows),
  };
}
