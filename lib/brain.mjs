...
function repairLooseJson(text) {
  ...
  // Trailing commas only — never strip array/object string values.
...
function tryParseObject(chunk) {
  try {
    ...
    // Double-encoded: "\"{...}\"" or '"{"silence":...}"'
    if (typeof parsed === "string") {
      return tryParseObject(parsed);
    }
  } catch {
    /* fall through */
  }
...
function parseJsonObject(text) {
  let raw = stripMarkdownFences(text);
  if (!raw) return null;

  // Whole-string parse (handles pretty JSON; rejects trailing junk).
  const direct = tryParseObject(raw);
  if (direct) return direct;

  // Double-encoded JSON string as the entire reply.
  if (/^\s*"/.test(raw)) {
    const once = tryParseObject(raw);
    if (once) return once;
  }
  ...
  // Granite sometimes appends an extra trailing `}` — brace match drops it.
  ...
  for (const chunk of candidates) {
    const parsed = tryParseObject(chunk);
