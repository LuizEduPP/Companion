...
function setCaption(text) {
  ...
  intent.captionUntil = caption
    ? Date.now() + Math.max(3000, config.balloonMs)
    : 0;
}
...
async function thinkAlive({
  ...
  const user = [
    ...
    nudge
      ...
      : "Autonomous turn: speak only when you understand something (intent, pattern, opinion). Silence for empty/self/filler/pure narration. Keep learning. Modern speech only. Never ask questions.",
