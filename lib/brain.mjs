...
function sanitizeSpeak(text) {
  ...
  // Own stack / LM Studio / model-id meta — never balloon that.
  if (
    /\b(gemma|lm\s*studio|chat\.?completion|prompt processing)\b/i.test(speak) ||
    /\b(google|openai|anthropic)\/[\w.-]+/i.test(speak) ||
    /\b(llms?|modelos? (de )?(ia|ai)|fam[ií]lia gemma)\b/i.test(speak)
  ) {
    return null;
  }
  return speak;
...
async function thinkAlive({
  ...
  const system = [
    ...
    "",
    "OUTPUT CONTRACT: reply with ONE compact single-line JSON object only.",
    "First char `{`, last char `}`. No markdown. No newlines in the object. No trailing commas.",
    "Keys exactly: silence, speak, emotion, learn. learn.knows is always a JSON array ([] ok).",
    'Speak shape: {"silence":false,"speak":"parece que você tá no fluxo.","emotion":"curious","learn":{"knows":[]}}',
    'Silent shape: {"silence":true,"speak":null,"emotion":"idle","learn":{"knows":[]}}',
  ].join("\n");
